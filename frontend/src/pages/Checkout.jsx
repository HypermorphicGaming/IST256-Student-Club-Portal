...
function Checkout() {
  ...
  const applyRegistrationToOpenSeats = () => {
    ...
    const updatedEvents = events.map((eventRecord) => {
      ...
      const isRegistered = cart.some(
        (item) => String(item.sourceEventId || item.productId) === eventId
      )
  ...
  const handleSubmit = async (e) => {
    ...
    if (!seatsUpdated) {
      ...
      setMessage({
        type: 'warning',
        text: 'One or more events are sold out. Please review open seats.',
      })
      return
    }
    ...
    try {
      ...
      // Clear cart and form after successful persistence.
      saveCart([])
      ...
      setMessage({
        type: 'warning',
        text: 'Registration was saved locally, but order history could not be updated.',
      })
    }
  }
  ...
  return (
    <PageShell>
      ...
      {message.text && (
        <div
          className={`alert alert-${message.type} alert-dismissible fade show position-fixed`}
